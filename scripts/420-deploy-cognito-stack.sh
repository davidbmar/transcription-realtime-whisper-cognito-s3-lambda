...
# Note: cognito-stack now includes full S3 file manager API (/api/s3/*, /api/audio/*)
# No need to look for external file manager stack
...
cd "$COGNITO_DIR"
cp "$REPO_ROOT/ui-source/app.js.template" web/app.js

# Replace placeholders with actual deployment values
sed -i.bak "s|TO_BE_REPLACED_USER_POOL_ID|$USER_POOL_ID|g" web/app.js
sed -i.bak "s|TO_BE_REPLACED_USER_POOL_CLIENT_ID|$USER_POOL_CLIENT_ID|g" web/app.js
sed -i.bak "s|TO_BE_REPLACED_IDENTITY_POOL_ID|$IDENTITY_POOL_ID|g" web/app.js
sed -i.bak "s|TO_BE_REPLACED_REGION|$AWS_REGION|g" web/app.js
...
log_success "Created web/app.js from template with deployment configuration"
